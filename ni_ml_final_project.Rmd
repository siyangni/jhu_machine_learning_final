...
By using LDA, we can achieve an overall accuracy of about 70%, as it is shown above. Not much improvement from the logistic model. Let's try another linear method: Quadratic Discriminant Analysis (QDA).
...
We can see that by using QDA, we can boost our prediction performance up to about 87%. However, if we want to do better, we must delve into more advanced machine learning models that could capture the nonlinearity of the data. 
...
From above, we can see that both the overall accuracy and the F1_score is unsatisfactory for the basic decision tree model. It is even lower than the linear models! Let's try the Random Forest Model.

```{r}
set.seed(688)

control_rf <- trainControl(method="cv", number=3, verboseIter=FALSE)
# It takes some times to run
...
We can see from above that with random forest model, given Number of variables available for splitting at each tree node (mtry) equals 24, we can reach a surprisingly high accuracy, which is over 99%. Note that in this project, we used the "ranger" package, which is a fast implantation of Random Forest. We have tried the classical "randomForest" package, but it takes too much comuting resources, which is beyond our equipment's computing ability. 

Let's try one last algorithm: the Gradient-Boosted Tree Model (not covered in the course content). Per our experience, the Gradient-Boosted Tree Model usually also have satisfactory prediction performance. 
...
Gradient-Boosted Tree Model has three tuning parameters: the number of trees (n.trees), the number of splits (interaction.depth), and learning rate (shrinkage). the train function automatically uses cross-validation to select the parameters for us that provide the best performance. From the above model fitting result, we can see that, if we train the model with 150 trees, 3 splits, and 0.1 learning rate, we can get a overall accuracy of around 99%. Another evaluation metric Cohen's Kappa Score (Kappa) also reaches 0.98. This is exciting! 

## Model Evaluation

However, we must be cautious, because we know that our top performers Gradient_Boosted Tree Model and Random Forest are prone to overfit in nature. Therefore, before we move any further, we now test our models on the calibration set we have left untouched up to this point.

First, let's see how Gradient_Boosted Tree Model performs on the calibration set:
...
The Gradient_Boosted Tree Model also gives out amazing prediction for the calibration set. The overall accuracy reaches 98.63%. The cohen's Kappa is about 0.986. Additionally, the F1 score is 0.997. Now let's see how Random Forest model perform on the calibration set:
...
From the above result we can also see that the Random Forest Model also performs almost perfectly on the calibration set, with a total accuracy rate of around 99.8%
...
From the above result, it is reasonable to believe that all the models we have built on the test data does not show significant performance downgrade once we fit them to the calibration data.

Finally, we build an ensemble based on majority vote. This simple ensemble will consult what each model predicts and then make its decision based on what the majority of all the models we have built so far vote for. For those observations where our six models cannot reach a 50% majority vote, we accept the predictions of the top performing model- Random Forest Model. 
...
The above result indicates that the majority vote ensemble model has an overall accuracy of about 97% and a Cohen's Kappa of about 0.96. This is a little inferior to the prediction performance of the Random Forest model and the Gradient-Boosted Tree Model. Therefore, for this final project, **we would recommend the Random Forest Model**, which slightly beat the Random Forest Model. However, we believe the ensemble model is less likely to overfit in the real world.

One last task is to fit this model to the test set we have reserved, so the quiz questions can be answered. This also provide an extra opportunity to test the reliability of our model of choice's performance.
...
Our algorithm got all the 20 quiz questions right. This again shows the Rnadom Forest Model we built can predict well.

## Conclusion

The above sections show that the Random Forest Model provides satisfactory prediction on individuals' exercise manner given their movement recorded by the accelerometers. **The overall accuracy given by the Random Forest Model on the calibration data reaches 99.8%.** However, given the algorithm of Random Forest Model tends to overfit in nature, we should expect the actual performance of our model be lower than 99.8%. **The ensemble Model based on the majority vote of all six models built in this project should be an ideal alternative to the Random Forest Model.** 
